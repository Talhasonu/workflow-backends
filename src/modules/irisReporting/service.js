...
const getReportPack = async ({ workspaceId }) => {
  ...
  return {
    ...
    recommendation:
      "Maintain a single evidence register per obligation and link each item to the appropriate report pack.",
  };
};
...
const updateRequirement = async ({ workspaceId, requirementId, payload }) => {
  ...
  if (payload.obligationType !== undefined)
    requirement.obligationType = payload.obligationType;
  if (payload.status !== undefined) requirement.status = payload.status;
  if (payload.dueDate !== undefined)
    requirement.dueDate = normalizeDueDate(payload.dueDate);
  if (payload.owner !== undefined) requirement.owner = payload.owner;
  if (payload.reportType !== undefined)
    requirement.reportType = payload.reportType;
  if (payload.materiality !== undefined)
    requirement.materiality = payload.materiality;
  if (payload.approvalRequired !== undefined)
    requirement.approvalRequired = Boolean(payload.approvalRequired);
  if (payload.evidenceRequired !== undefined)
    requirement.evidenceRequired = Array.isArray(payload.evidenceRequired)
      ? payload.evidenceRequired
      : [];
...
const deleteRequirement = async ({ workspaceId, requirementId }) => {
  const requirement = await IrisReportingRequirement.findOneAndDelete({
    _id: requirementId,
    workspaceId,
  });
  if (!requirement) {
...
const uploadEvidenceFile = async ({
  ...
  if (cloudinaryReady) {
    ...
    console.warn(
      "[IRIS] Cloudinary not configured — storing file as base64 in DB (dev fallback)",
    );
...
const deleteEvidenceFile = async ({ workspaceId, requirementId, fileId }) => {
  ...
  const file = requirement.evidenceFiles.find(
  ...
  requirement.evidenceFiles = requirement.evidenceFiles.filter(
    (f) => f._id.toString() !== fileId,
  );
  await requirement.save();
